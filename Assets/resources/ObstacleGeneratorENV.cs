...
public class ObstacleGeneratorENV : MonoBehaviour {
/* ========================================================================================================
 ...
 */
    ...
    IEnumerator ObstacleGeneration()
	{
        ...
        if (prevObstacle != null)
        {
            ...
            float nextPosition = prevObstacle.transform.position.z + prevObstacle.GetComponent<BoxCollider>().bounds.size.z;
            
             if(nextPosition > 40)
                yield return ObstacleGeneration();
               
            castPositionSet = new Vector3(castPositionSet.x, castPositionSet.y, nextPosition);
            //Debug.Log(castPositionSet);
        }
        ...
        prevObstacle = obstacleCasted;
        
        yield return ObstacleGeneration();
    }
    public static float getObjSize(GameObject prevObstacle, float min, float max)
    {
        if (prevObstacle.GetComponent<MeshCollider>() != null)
        {
            return Mathf.Abs(prevObstacle.GetComponent<MeshCollider>().bounds.max.z 
                - prevObstacle.GetComponent<MeshCollider>().bounds.min.z);
        }
        else {
            float minB = min, maxB = max ;
            foreach(MeshCollider mesh in prevObstacle.GetComponentsInChildren<MeshCollider>())
            {
                
                float minC = mesh.bounds.min.z;
                float maxC = mesh.bounds.max.z;
