 ...
 */
    ...
    IEnumerator ObstacleGeneration()
	{
		...
		for (int w = 0; w < actualWaves; w++)
		{
            ...
            for (int i = 0; i < numObstacles; i++)
			{
                ...
                {
                    castPositionSet = new Vector3(castPositionSet.x, castPositionSet.y, prevObstacle.transform.position.z+(prevObstacle.GetComponent<BoxCollider>().bounds.size.z));
    ...
    public float getObjEnd(GameObject prevObstacle, float closest)
    {
        //Debug.Log(prevObstacle.name);
        if (prevObstacle.GetComponent<BoxCollider>() == null)
        {
            
            foreach (Transform child in prevObstacle.transform)
            {
                float newClosest = getObjEnd(child.gameObject, closest);
            ...
            Component[] meshes = prevObstacle.GetComponentsInChildren(typeof(BoxCollider));
            for (int i =0;i< meshes.Length;i++)
            {
                float newClosest = meshes[i];
                if (newClosest < closest)
                {
                    closest = newClosest;
                }
        ...
        {
            if (prevObstacle.GetComponent<BoxCollider>().bounds.max.z > closest)
            {
                
                closest = prevObstacle.GetComponent<BoxCollider>().bounds.max.z;
                //Debug.Log(closest);
            }
        }
        return closest;
